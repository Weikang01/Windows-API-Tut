...
int CALLBACK WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInst, _In_ LPSTR args, _In_ int ncmdshow)
{
	...
	hWindow = CreateWindowA("myWindowClass", "My Window", WS_VISIBLE | WS_OVERLAPPEDWINDOW, 100, 0, 600, 800, NULL, NULL, NULL, NULL);
...
void writeFile(LPSTR path)
{
	FILE* file;
	file = fopen(path, "w");

	int _size = GetWindowTextLengthA(hFile);
	char* data = new char[_size + 1];
	GetWindowTextA(hFile, data, _size + 1);
...
void AddControls(HWND hWnd)
{
	...
	CreateWindowA("Button", "Save File", WS_VISIBLE | WS_CHILD | WS_TABSTOP, 250, 380, 98, 38, hWnd, (HMENU)SAVE_FILE, NULL, NULL);

	hFile = CreateWindowA("Edit", NULL, WS_VISIBLE | WS_CHILD | WS_BORDER | ES_MULTILINE | WS_VSCROLL | WS_HSCROLL, 100, 430, 400, 300, hWnd, NULL, NULL, NULL);
}
